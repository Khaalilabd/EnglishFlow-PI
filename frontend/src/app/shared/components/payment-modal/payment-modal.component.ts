...
export class PaymentModalComponent implements OnInit, OnDestroy {
    ...
    this.paymentService.initiate(req).subscribe({
      ...
      error: (err) => {
        ...
        if (err?.status === 409 || msg.toLowerCase().includes('already enrolled')) {
          // Already paid — trigger enrollment directly and show success
          this.success = true;
          this.enrolled.emit();
        } else {
  ...
  private startPopupCheck(): void {
    this.clearPopupCheck();
    // Poll every second — when popup closes, verify payment status automatically
    this.popupCheckInterval = setInterval(() => {
      if (this.popup?.closed && !this.verifying && !this.success) {
        this.clearPopupCheck();
        this.waitingForPopup = false;
        // Auto-verify when popup closes — covers the case where paymee.complete
  ...
  private onPaymeeComplete(): void {
    ...
    // paymee.complete fires only on successful payment — mark as success directly
